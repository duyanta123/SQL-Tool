import type { DataFlowEdge, DataFlowGraph, DataFlowNode, DFEdgeKind } from '@/types/dataflow';
import { asArray, asNode, getColumnRef, getCtes, getFromItems, getSubquery, identifier, normalizeTableRef, outputColumns, statementType, walkAst, type AstNode } from './ast-normalizer';
...
function addEdge(ctx: Context, source: string, target: string, kind: DFEdgeKind, label?: string, joinType?: string): void {
  if (!source || !target || source === target) return;
  const signature = `${source}|${target}|${kind}|${label ?? ''}`;
  if (ctx.edges.has(signature)) return;
  ctx.edges.set(signature, { id: `df-edge::${ctx.edges.size + 1}`, source, target, kind, label, joinType });
}
...
function targetNode(ctx: Context, statementId: string, operation: 'INSERT' | 'UPDATE' | 'CREATE' | 'DELETE', tableName: string, columns: string[] = []): string {
  return addNode(ctx, {
...
function processSelect(select: AstNode, ctx: Context, statementId: string, inheritedCtes: Map<string, string>): string {
  ...
  for (const item of getFromItems(select)) {
    ...
    if (nested) {
      ...
      if (alias) outerAliases.add(alias.toLowerCase());
      continue;
    }
    ...
    outerAliases.add((alias || ref.alias || ref.table).toLowerCase());
  }

  const groups = asArray(select.groupby);
  ...
  if (groups.length || aggregates.length) {
    const groupColumns = groups.map(item => getColumnRef(item)?.column ?? identifier(asNode(item)?.column ?? item)).filter(Boolean);
    outputId = addNode(ctx, {
      ...
      groupByColumns: groupColumns, aggregateFunctions: aggregates, detail: groupColumns.length ? `GROUP BY ${groupColumns.join(', ')}` : undefined,
    });
  } else {
    outputId = addNode(ctx, {
      id: nextId(ctx, `${statementId}::result`), kind: 'target', label: '查询结果', statementId,
      operation: 'SELECT', outputColumns: outputColumns(select),
    });
  }
  ctx.processed.set(select, outputId);

  sources.forEach(source => addEdge(ctx, source.id, outputId, source.join ? 'join' : 'read', source.join ? normalizeJoinLabel(source.join) : undefined, source.join));
...
function processInsert(statement: AstNode, ctx: Context, statementId: string): void {
  ...
  if (!ref) return;
  const target = targetNode(ctx, statementId, 'INSERT', ref.id, asArray(statement.columns).map(identifier).filter(Boolean));
  const select = asNode(statement.values);
  if (select && statementType(select) === 'select') {
    addEdge(ctx, processSelect(select, ctx, statementId, new Map()), target, 'write', 'INSERT');
    return;
  }
  ...
  addEdge(ctx, literal, target, 'write', 'INSERT');
}
...
function processCreate(statement: AstNode, ctx: Context, statementId: string): void {
  if (identifier(statement.keyword).toLowerCase() !== 'table') return;
  const ref = normalizeTableRef(statement.table);
  if (!ref) return;
  const target = targetNode(ctx, statementId, 'CREATE', ref.id);
  const query = asNode(statement.query_expr ?? statement.as);
  if (query && statementType(query) === 'select') addEdge(ctx, processSelect(query, ctx, statementId, new Map()), target, 'write', 'CTAS');
}
