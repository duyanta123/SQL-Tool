...
describe('表达式级 JOIN/WHERE 可视化 expression display', () => {
  it('JOIN 条件保留 AND/OR 与括号分组并解析别名', () => {
    const result = parseSQL("SELECT u.id FROM users u JOIN orders o ON u.id = o.user_id AND (o.amount > 100 OR o.status = 'vip')", 'mysql');
  ...
  it('结果节点 detail 展示 WHERE 文本', () => {
    const result = parseSQL("SELECT u.id FROM users u WHERE u.age > 18 AND u.name LIKE 'A%'", 'mysql');
    expect(result.error).toBeNull();
  ...
  it('WHERE 中的过滤列归属到对应 JOIN 来源', () => {
    const result = parseSQL('SELECT u.id FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount > 5', 'mysql');
    const target = resultNode(result.dfGraph);
  ...
  it('HAVING 文本进入聚合节点 detail', () => {
    const result = parseSQL('SELECT user_id FROM orders GROUP BY user_id HAVING COUNT(*) > 1', 'mysql');
    expect(result.error).toBeNull();
  ...
  it('IN 列表渲染为可读文本', () => {
    const result = parseSQL('SELECT u.id FROM users u WHERE u.id IN (1, 2, 3)', 'mysql');
    expect(result.error).toBeNull();
    const target = resultNode(result.dfGraph);
    expect(target?.detail).toContain('IN (1, 2, 3)');
  });

  it('未限定的过滤列不归属任何边（不猜测）', () => {
    const result = parseSQL("SELECT o.id FROM orders o JOIN users u ON o.user_id = u.id WHERE status = 'paid'", 'mysql');
    expect(result.error).toBeNull();
    const target = resultNode(result.dfGraph);
